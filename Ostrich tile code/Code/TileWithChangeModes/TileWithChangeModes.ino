...
//      FastLED.delay(1000 / FRAMES_PER_SECOND);
    ...
    if (String((char*)buf) == ("L2")) { //qrs*****************************this will only run if it receives a message for this specific tile, change 1 to diff int for diff tiles
//      leds = true; //turn on LEDs if receives L1
    ...
    else if (String((char*)buf) == ("L1") or String((char*)buf) == ("L3") or String((char*)buf) == ("L4")) {//qrs***********
//      leds = false;
...
//  if(testPressure() and !prevPressure or !digitalRead(button) and prevButton)
  if(testPressure() and !prevPressure) { //does this test if it's being pressed or not being pressed??????????    
    ...
    uint8_t data[] = "B2"; //qrs*****************************change based on tile number CHANGE FOR EACH TILE
